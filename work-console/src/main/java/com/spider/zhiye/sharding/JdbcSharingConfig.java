package com.spider.zhiye.sharding;
...
public final class JdbcSharingConfig implements SingleKeyTableShardingAlgorithm<String> {
    /**
     * doEqualSharding =
     *
    ...
    public String doEqualSharding(final Collection<String> tableNames, final ShardingValue<String> shardingValue) {
        ...
        int index = (int) (shardingValue.getValue().hashCode() % 450);
        String indexStr = "";
        if (index >= 300)
            indexStr = "2";
        else if (index >= 150)
    ...
    public Collection<String> doInSharding(final Collection<String> tableNames, final ShardingValue<String> shardingValue) {
        ...
        for (String value : shardingValue.getValues()) {
            int index = (int) (value.hashCode() % 450);
            String indexStr = "";
            if (index >= 450)
                indexStr = "2";
            else if (index >= 300)
    ...
    public Collection<String> doBetweenSharding(final Collection<String> tableNames, final ShardingValue<String> shardingValue) {
        ...
        for (int i = range.lowerEndpoint().hashCode(); i <= range.upperEndpoint().hashCode(); i++) {
            int index = (int) (i % 450);
            String indexStr = "";
            if (index >= 450)
                indexStr = "2";
